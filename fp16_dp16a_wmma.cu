...
void executeWMMA(const TestCase* testCases, Result* results, int numTests) {
    ...
    cudaMalloc(&d_A, numTests * matrixSize * sizeof(__half));
    cudaMalloc(&d_B, numTests * matrixSize * sizeof(__half));
    ...
    cudaMemcpy(d_A, h_A.data(), h_A.size() * sizeof(__half), cudaMemcpyHostToDevice);
