...
#define CHECK_LAST_CUDA_ERROR() { \
    ...
    if (err != cudaSuccess) { \
        std::cerr << "CUDA Kernel Launch Error: " << cudaGetErrorString(err) << " at line " << __LINE__ << std::endl; \
        exit(1); \
    } \
}
// ----------------
...
void executeWMMA(const TestCase* testCases, Result* results, int numTests) {
    ...
    CHECK_CUDA(cudaMalloc(&d_A, numTests * matrixSize * sizeof(__half)));
    CHECK_CUDA(cudaMalloc(&d_B, numTests * matrixSize * sizeof(__half)));
    CHECK_CUDA(cudaMalloc(&d_C, numTests * sizeof(float)));
    CHECK_CUDA(cudaMalloc(&d_D, numTests * matrixSize * sizeof(float)));
